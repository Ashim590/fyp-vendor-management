...
function getTransporter(): Transporter | null {
  ...
  if (
    process.env.NOTIFICATION_EMAIL === "0" ||
    process.env.NOTIFICATION_EMAIL === "false"
  ) {
    transporter = null;
    return null;
  }
  ...
  transporter = nodemailer.createTransport({
    ...
    auth:
      user && pass != null && pass !== "" ? { user, pass: pass } : undefined,
  });
...
export async function maybeSendNotificationEmail(
  ...
  try {
    ...
    const fromAddr =
      process.env.SMTP_FROM?.trim() || process.env.MAIL_FROM?.trim();
    if (!fromAddr) return;
    ...
    const fromName =
      process.env.MAIL_FROM_NAME?.trim() || "Paropakar VendorNet";
