...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const cfg = getEsewaConfig();
    if (!cfg.secretKey) {
      return res.status(500).json({ message: 'eSewa secret key not configured' });
    }
    ...
    const freshTransactionUuid = generateEsewaTransactionUuid('inv', String(payment._id));
...
router.all('/esewa/success', async (req: AuthRequest, res: Response) => {
  const cfg = getEsewaConfig();
  const encoded =
    (req.query as { data?: string }).data || (req.body as { data?: string })?.data || '';
  ...
  try {
    ...
    if (!transactionUuid) return res.status(400).send('Missing transaction_uuid');
    ...
    if (['CANCELED', 'NOT_FOUND', 'FULL_REFUND', 'PARTIAL_REFUND'].includes(esewaStatus)) {
